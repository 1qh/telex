...
const white = '#ffffff'
const yellow = '#ffff00'
const drawVietnam = (ctx: OffscreenCanvasRenderingContext2D, size: number) => {
  ctx.fillStyle = red
  ctx.fillRect(0, 0, size, size)
  ctx.fillStyle = yellow
  star(ctx, size)
}
const drawUk = (ctx: OffscreenCanvasRenderingContext2D, size: number) => {
  const half = size / 2
  const diagWhite = size * 0.3
  const diagRed = size * 0.18
  const crossWhite = size * 0.32
  const crossRed = size * 0.18
  ctx.fillStyle = blue
  ctx.fillRect(0, 0, size, size)
...
const updateBadge = async (tabId: number) => {
  ...
  await browser.action.setTitle({
    tabId,
    title: enabled ? 'Double Option/Alt to switch between simple and worker mode\n' : 'Click or Double Ctrl to toggle\n'
  })
  if (!(enabled && worker)) {
...
const sendToNative = (message: { action: string; deleteCount?: number; insertText?: string; usePaste?: boolean }) => {
  if (!nativePort) connectNative()
  try {
    // oxlint-disable-next-line unicorn/require-post-message-target-origin -- chrome runtime Port.postMessage has no target-origin parameter (not window.postMessage)
...
const tabEngines = new Map<number, ReturnType<typeof createEngine>>()
const getTabEngine = (tabId: number) => {
...
const toneCombining = /[\u0300\u0301\u0303\u0309\u0323]/u
const markCombining = /[\u0302\u0306\u031B]/u
const needsPaste = (text: string) => {
...
browser.runtime.onMessage.addListener(
  ...
  (message: ContentMessage, sender, sendResponse: (response: unknown) => void): boolean => {
    ...
    if (message.type === 'worker_key') {
      ...
      const before = engine.getProcessedString()
      ...
      const insertText = after.slice(commonPrefix)
      const pasteMode = insertText ? needsPaste(insertText) : false
      sendToNative({ action: 'type', deleteCount, insertText, usePaste: pasteMode })
...
const setIcon = async (value: boolean) => {
